 ...
 * */
int decrease_count(int count, int thread_index) {
    ...
    if (available_resources < count) {
        ...
        printf("Locked %i resources in thread %i, now available: %i\n" , count , thread_index , available_resources);
        sem_post(&semaphore);
        return 0;
    }
}
 ...
 *
 * */
int increase_count(int count, int thread_index) {
    if (count + available_resources > 5) {
        return -1;
    } else {
        sem_wait(&semaphore);
        available_resources += count;
        printf("Freed %i resources in thread %i, now available: %i\n" , count , thread_index , available_resources);
        sem_post(&semaphore);
        return 0;
    }
}
...
void *runTimes(void *null) {
    ...
    while (i < times) {
        printf("Thread %i\n", (int) null);
        result = -1;
        while (result < 0) {result = decrease_count(1,(int) null);}
        result = -1;
        while (result < 0) {result = increase_count(1, (int) null);}
        i += 1;
    }
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&thread0,NULL,runTimes,(int*) 1);
    pthread_create(&thread1,NULL,runTimes,(int*) 2);
